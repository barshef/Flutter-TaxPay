...
{
    public partial class RequerryFailTransaction : System.Web.UI.Page
    {
        ...
        private void Geturl()
        {
            ...
            bc = CrossCheckResponse(Session["transidno"].ObjectToString());
            if (bc.IsSuccessful)
            {
                var dBtransId = Session["dBtransId"].ObjectToString();
                var dbStateCode = Session["StateCode"].ObjectToString();
                Session["transidno"] = dBtransId;
                Session["StateCode"] = dbStateCode;
            ...
            else
            {
                ...
                Session["transidno"] = dBtransId;
                Session["StateCode"] = dbStateCode;
                Session["failmsg"] = bc.StatusMessage;
        ...
        private MessageClass CrossCheckResponse(string transRefsNo)
        {
            ...
            if (dtresult.Tables[0].Rows.Count > 0)
            {
                ...
                if (dbTranStatus == "")
                {
                    bc = AccessPayGate(dBtransId);

                }
                else if (dbTranStatus == "False")
                {
                    bc.StatusId = -1;
                    bc.StatusMessage = Session["dbTranStatusMgs"].ObjectToString();
                }
                else
                {
                    bc.StatusId = 1;
            ...
            {
                bc.StatusMessage = "Record not found found database";
                var ex = new Exception($"Record not found from database - {bc.StatusMessage}");
                CreateLogFiles.ExceptionLogging.SendErrorTomail(ex,
                    HttpContext.Current.Request.Url.ToString());
                bc.StatusId = -1;
            }
        ...
        private MessageClass AccessPayGate(string raveTransvalue)
        {
            var bc = new MessageClass();
            var gateWayId = Session["DbGateWayID"].ToString();
            if (gateWayId == Global.Raveconfigs.RaveSwitchGateWayId)
            {
                Session["RevenueName"] = Session["dbRevenueName"].ObjectToString();
                Session["txtpayeamt"] = Session["transDbAmount"].ObjectToString();
                bc = VerifyRaveResponse(raveTransvalue);
            }
            else
            {
        ...
        private MessageClass CompareResponse(string raveTransIdValue)
        {
            var bc = new MessageClass();
        ...
        private MessageClass SendGudTransDetailsEmail(string dBtransId)
        {
            ...
            if (dtresult.Tables[0].Rows.Count > 0)
            {
                if (!string.IsNullOrEmpty(Session["DbGateWayID"].ToString()) || !string.IsNullOrEmpty(Session["StateCode"].ToString()))
                {
                    ...
                    var gudreturnmsg = "Transaction Successful";
                    ...
                    mailbody = mailbody.Replace("##ActivationLink##", "'" + Request.Url.AbsoluteUri.Replace("ProcessFailTransaction", "Tiers/RequerryTransaction.aspx?ActivationCode=" + Convert.ToString(dtresult.Tables[0].Rows[0]["TransRefNo"])) + "'");
                ...
                if (!string.IsNullOrEmpty(Session["DbGateWayID"].ToString()) || !string.IsNullOrEmpty(Session["StateCode"].ToString()))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(to))
                    {
                        ...
                        catch (Exception exception)
                        {
                            ...
                            var err = new CreateLogFiles();
                            err.ErrorLog(Server.MapPath("~/Logs/ErrorLog"), exception.Message);
                            CreateLogFiles.ExceptionLogging.SendErrorTomail(exception, HttpContext.Current.Request.Url.ToString());
                            bc.StatusMessage = "Error sending mail to your specified email address";
                        }
                    }
                    else
